...
- 🔧 **SQL 格式化改进**：修复了 MyBatis XML 文件中多个 SQL 格式化问题
  - **SET 子句缩进**：修复了 `SET` 后只有第一列正确缩进，后续列丢失缩进的问题
    - 根本原因：CST 解析器按 MyBatis 参数（`#{...}`）分割 SQL，导致每个片段独立格式化
    - 解决方案：使用占位符方案——格式化前提取参数，格式化完整 SQL，然后恢复参数
  - **括号对齐**：修复了动态 SQL 中不完整片段的括号对齐问题
    - 例如：`<if>` 标签内的 `AND\n(` 现在保持一致的缩进
    - 根本原因：当 sql-formatter 无法解析不完整的 SQL 片段时，错误处理程序只缩进第一行
    - 解决方案：在成功和失败路径中都对所有行应用正确的缩进
  - **CDATA 块保留**：修复了 CDATA 块被错误格式化的问题
    - 像 `<![CDATA[ date >= #{start} AND date < #{end} ]]>` 这样的 CDATA 内容现在保持为单行
    - 解决方案：格式化前提取 CDATA 块，格式化后恢复
  - **XML 注释保留**：修复了 XML 注释在格式化过程中被损坏的问题
    - 像 `<!--SELECT * FROM t_data-->` 这样的注释被错误地拆分为 `< ! --SELECT...`
    - 解决方案：在任何处理之前提取 XML 注释，格式化后恢复
  - 添加了 12 个新的单元测试，覆盖所有格式化修复

## [0.3.11] - 2026-01-20

### 新增

- ⚙️ **参数验证开关**：添加新配置选项 `mybatis-boost.enableParameterValidation`，用于启用/禁用 MyBatis XML 文件中的实时参数验证
  - **默认值**：`true`（启用）
  - **实时切换**：配置更改立即生效，无需重新加载扩展
  - **启用时的行为**：验证 `#{param}` 和 `${param}` 引用是否与 Java 方法参数和实体字段匹配
  - **禁用时的行为**：清除所有参数验证诊断并跳过验证
  - **使用场景**：适用于具有自定义参数模式的项目，或在验证导致性能问题时使用
  - 监听 `onDidChangeConfiguration` 事件以实现即时更新
  - 添加了公共 `isEnabled()` 方法用于调试和测试
  - 添加了 8 个全面的单元测试用于配置切换功能
